...
export default {
  ...
  methods: {
    async onDelete() {
      ...
      this.error = null;
      try {
        const res = await deletePortfolio({ id: this.show })
        if (res.ok) {
          this.$emit("on-close");
        } else {
  ...
  template: html`
    ...
    >
      <div class="cdf-modal-dialog cdf-modal-dialog--sm" @mousedown.self="onCancel">
        <div class="cdf-modal-content">
          <div class="cdf-modal-header">
            <div class="cdf-modal-header__left">
              <div class="cdf-modal-title">Delete Portfolio</div>
            </div>
          ...
          <div class="cdf-modal-body">
            Do you want to delete your Portfolio? Performing this action you
            understand that you would not be able to recover your Portfolio.
            <div v-if="error" class="cdf-error-banner">
              {{ error }}
            </div>
          </div>
          <!-- .cdf-modal-body -->
          <div class="cdf-modal-footer">
            <div class="cdf-grid cdf-grid--cols-2">
              <button class="cdf-btn cdf-btn--danger" type="button" @click="onDelete" :disabled="loading">
